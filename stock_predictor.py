...
try:
    ...
    print("❌ Could not load tickers:", e)
    TICKERS = []

def predict_stock(user_input, forecast_days=30):
    """
    Predict stock prices for a given symbol or company name.
    Returns a base64-encoded plot and a pandas Series of forecasted prices.
    """
    try:
        tickers_df = pd.read_csv(TICKER_PATH)
    except:
    ...
    # Try matching by symbol or company name
    match = tickers_df[
